...
async def upload_listing_galery(page: Page, image_name, recur=0):
    try:
        if image_name:
            input_selector = '//*[@id="job_gallery"]'
            image_path = f"{AppConfig.PARENT_DIRECTORY_PROJECTS_MAIN_FILE}/{AppConfig.PICTURES_DIRECTORY}/{image_name}"
            print(f"importer ->step 12 upload_listing_logo({image_path})")
        ...
        if recur < 3:
            await upload_listing_galery(page, image_name, recur + 1)
