...
class RuntimeResource(metaclass=Singleton):
    ...
    def __init__(self):
        print("importer ->initialising resource ...")
    ...
    async def initialize_tabs(self):
        self.pages.append(
            await (
                await self.browser.new_context(user_agent=random.choice(_CHROME))
            ).new_page()
        )
        self.pages.append(
            await (
    ...
    async def free(self):
        try:
            ...
            print(f"importer ->Failed to stop browser: {e}")

        try:
            print("importer ->Stopping browser ...")
            await self.browser.close()
        except Exception as e:
            print(f"importer ->Failed to stop browser: {e}")
        self.browser = None
        try:
            print("importer ->Stopping Playwright ...")
            await self.playwright.stop()
        except Exception as e:
            print(f"importer ->Failed to stop Playwright: {e}")
